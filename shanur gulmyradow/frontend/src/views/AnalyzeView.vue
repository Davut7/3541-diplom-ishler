...
export default {
  ...
  setup(props) {
    ...
    const loadSampleData = async () => {
      ...
      try {
        const response = await fetch(`${API_URL}/packets?count=100`)
        const data = await response.json()
        if (data.success) {
          const packets = []
          for (let batch = 0; batch < 5; batch++) {
            const batchResponse = await fetch(`${API_URL}/packets?count=100`)
            const batchData = await batchResponse.json()
            packets.push(...batchData.packets.map((p, i) => ({ ...p, no: batch * 100 + i + 1 })))
          }
    ...
    const loadLargeSampleData = async () => {
      ...
      try {
        ...
        for (let batch = 0; batch < 20; batch++) {
          const response = await fetch(`${API_URL}/packets?count=100`)
          const data = await response.json()
          packets.push(...data.packets.map((p, i) => ({ ...p, no: batch * 100 + i + 1 })))
        }
        fileContent.value = packets
        packetCount.value = packets.length
        uploadedFile.value = { name: 'large_capture.json', size: JSON.stringify(packets).length }
      } catch (error) {
        console.error('Error loading large sample data:', error)
      } finally {
        isLoadingSample.value = false
      }
    }
